...
public class Fox_Sniffing : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        for (int i = 0; i < _feathers.Count; i++)
        {
            ...
            {
                _featherDirections.Add((_feathers[i].gameObject.transform.position - transform.position) * Mathf.Pow(_feathers[i]._smell, 3));
            }
            else
            {
                _featherDirections.Add(Vector3.Normalize(_feathers[i].gameObject.transform.position - transform.position) * Mathf.Pow(_feathers[i]._smell, 3));
            }
        }
